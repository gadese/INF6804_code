...
class SQLHandler:
    ...
    def select_tracking_info(connection):
        ...
        r = connection.cursor().execute(query)
        for q in r:
            print(q)
...
class MOTMetrics:
    ...
    def compute_metrics(self, first_instant, last_instant, threshold, IoU):
        ...
        return motp, mota, correct_tracks, misses, false_positives, mismatches


def main(data_hypotheses, data_annotations, min_frame, max_frame):

    mot_metrics = MOTMetrics(data_annotations, data_hypotheses)
    motp, mota, correct, misses, fp, mismatch = mot_metrics.compute_metrics(min_frame, max_frame, 0.5, True)

    print('MOTP = {:.4f}, MOTA = {:.4f}, Correct = {}, Misses = {}, FP = {}, Mismatch = {}'.format(motp, mota, correct, misses, fp, mismatch))
